...
export const GameSummaryModal: React.FC<GameSummaryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in print:p-0 print:bg-white">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden print:border-none print:shadow-none print:max-h-full print:bg-white print:text-black">
        {/* Modal Top Header */}
        <div className="p-3.5 sm:p-4 border-b border-slate-800 flex items-center justify-between gap-3 shrink-0 print:hidden bg-slate-950/50">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-base sm:text-lg font-bold text-white truncate flex items-center gap-2">
                <span>比赛战报与统计总览</span>
                {exportSuccessMsg && (
                  <span className="text-xs font-normal text-emerald-400 bg-emerald-950/80 px-2 py-0.5 rounded-full border border-emerald-500/30 animate-in fade-in">
                    {exportSuccessMsg}
                  </span>
                )}
              </h2>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <div className="relative" ref={exportDropdownRef}>
              <button
                ...
                className="px-2.5 sm:px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-md shadow-amber-500/10 cursor-pointer active:scale-95"
              >
                {isExportingImage ? (
            ...
            <button
              onClick={onClose}
              className="p-1.5 sm:p-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors cursor-pointer"
            >
        ...
        {/* View Tabs */}
        <div className="px-4 pt-2.5 pb-1 border-b border-slate-800 bg-slate-950/30 flex items-center gap-2 shrink-0 print:hidden text-xs">
          <button
